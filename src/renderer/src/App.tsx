...
import { defaultFilters, defaultSettings, mediaTypeOf, titleKey } from '../../shared/types'
import Discover from './views/Discover'
import ForYou from './views/ForYou'
import Library from './views/Library'
import SettingsView from './views/Settings'
import Inspector from './components/Inspector'
import { IconForYou, IconLibrary, IconSearch, IconSettings } from './components/Icons'
...
export default function App(): JSX.Element {
  ...
  const genres = mediaTypeOf(filters.titleKind) === 'tv' ? tvGenres : movieGenres
  ...
  useEffect(() => {
    ...
    window.api
      ...
      .catch(() => {
        if (!cancelled && detailsCache.current.get(key) == null) setDetails(null)
      })
    return () => {
      cancelled = true
    }
  ...
  const inspector = (
    <Inspector
      movie={selected}
      details={details}
      entry={library.find((e) => selected && titleKey(e) === titleKey(selected))}
      match={'match' in (selected ?? {}) ? (selected as { match?: number }).match : null}
      docked={!discoverLayout}
      genreMap={genreMap}
      onUpsert={upsert}
      onRemove={async (id, mediaType) => {
        setLibrary(await window.api.removeLibrary(id, mediaType))
        if (selected && titleKey(selected) === titleKey({ tmdbId: id, mediaType })) setSelected(null)
      }}
    />
  )
    ...
    <div className="grid h-full grid-rows-[36px_1fr]">
      ...
      >
        <nav
          ...
          aria-label="Primary"
        >
          <div className="mb-2 grid size-[38px] place-items-center rounded-xl bg-gradient-to-b from-accent-2 to-accent text-xs font-bold tracking-[-0.06em] text-accent-ink shadow-accent">
            IB
          </div>
          <NavBtn id="discover" view={view} setView={setView} label="Search" tip="Search">
            <IconSearch />
          </NavBtn>
          <NavBtn id="foryou" view={view} setView={setView} label="For you" tip="For you">
            <IconForYou />
          </NavBtn>
          <NavBtn id="library" view={view} setView={setView} label="Library" tip="Library">
            <IconLibrary />
          </NavBtn>
          <NavBtn id="settings" view={view} setView={setView} label="Settings" tip="Settings">
            <IconSettings />
          </NavBtn>
        </nav>
        <main
        ...
        >
          {error ? (
            <div
              className={cn(
                'mb-3.5 rounded-app border border-[color:var(--color-danger-border)] bg-[color:var(--color-danger-bg)] px-3 py-2.5 text-danger-fg',
          ...
          {booting ? (
            <div className="px-4 py-9 text-center text-muted">Loading your ranking studio…</div>
          ) : showWelcome ? (
            <section className="mx-auto my-[8vh] max-w-[640px] px-0 py-2">
              <h2 className="mt-0 mb-2.5 text-[32px] tracking-title">Start with a TMDB key.</h2>
        ...
        {!discoverLayout && selected && view !== 'settings' && !showWelcome && !booting ? inspector : null}
      </div>
    </div>
  )
}
