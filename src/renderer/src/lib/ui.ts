...
export function btn(...variants: Array<'primary' | 'gold' | 'ghost' | 'danger' | 'link' | false | undefined>): string {
  ...
  return cn(
    'app-no-drag rounded-full border px-3.5 py-2 text-[13px] font-semibold transition-colors duration-140 disabled:cursor-not-allowed disabled:opacity-45',
    primary
      ...
      : danger
        ? 'border-(--color-danger-ring) bg-transparent text-danger hover:bg-white/9'
        : ghost
          ? 'border-line bg-transparent text-ink hover:bg-white/9'
          : 'border-line bg-white/5 text-ink hover:bg-white/9',
    variants.includes('link') && 'inline-flex items-center no-underline'
  )
}

export function iconBtn(...variants: Array<'primary' | 'busy' | false | undefined>): string {
  return cn(
    'grid size-10 shrink-0 place-items-center rounded-xl border p-0',
    variants.includes('primary')
      ? 'border-transparent bg-linear-to-b from-accent-2 to-accent text-accent-ink shadow-accent hover:from-accent-2 hover:to-accent hover:text-accent-ink'
      : 'border-line bg-white/5 text-muted hover:bg-white/9 hover:text-ink',
...
export function rankedRow(active = false, extra?: string): string {
  return cn(
    ...
    active ? 'inset-accent bg-accent-soft hover:bg-accent-soft' : 'bg-transparent hover:bg-white/3',
    extra
  )
}
