...
export function applyMovieEnrichment(movie: RankedMovie, patch: MovieEnrichment): RankedMovie {
  return {
    ...movie,
    runtime: movie.runtime ?? patch.runtime,
    certification: movie.certification ?? patch.certification,
    directorIds: movie.directorIds?.length ? movie.directorIds : patch.directorIds,
    directorNames: movie.directorNames?.length ? movie.directorNames : patch.directorNames,
...
export function titleKey(title: { tmdbId: number; mediaType?: MediaType }): string {
...
export function sortMovies<T extends MovieSummary & { match?: number }>(movies: T[], sortBy: string): T[] {
...
function movieComparator(
  sortBy: string
): ((a: MovieSummary & { match?: number }, b: MovieSummary & { match?: number }) => number) | null {
  const byId = (a: MovieSummary, b: MovieSummary): number =>
    titleKey(a).localeCompare(titleKey(b)) || a.tmdbId - b.tmdbId
  switch (sortBy) {
    case 'match':
      return (a, b) => n(b.match) - n(a.match) || n(b.voteAverage) - n(a.voteAverage) || n(b.voteCount) - n(a.voteCount) || byId(a, b)
    ...
    case 'vote_count.desc':
      return (a, b) => n(b.voteCount) - n(a.voteCount) || n(b.voteAverage) - n(a.voteAverage) || byId(a, b)
    case 'popularity.desc':
      return (a, b) => n(b.popularity) - n(a.popularity) || byId(a, b)
    case 'primary_release_date.desc':
      return (a, b) => (b.releaseDate || '').localeCompare(a.releaseDate || '') || byId(a, b)
    case 'primary_release_date.asc':
      return (a, b) => (a.releaseDate || '').localeCompare(b.releaseDate || '') || byId(a, b)
    default:
      return null
  }
}
