...
import { DATA_DIR, DATASET_FILE, DATASET_URL, SYNC_INTERVAL_MS } from '../config.js'
...
async function ensureDataset(store: RatingsStore, force: boolean): Promise<void> {
  ...
  try {
    ...
    if (store.ready()) {
      console.warn('IMDb ratings refresh failed; keeping the last loaded dataset.', error)
      return
    }
    throw error
  }
...
async function downloadDataset(file: string): Promise<void> {
  const response = await fetch(DATASET_URL, { headers: { Accept: 'application/gzip' } })
...
export async function parseRatingsTsv(file: string): Promise<Map<string, ImdbRating>> {
