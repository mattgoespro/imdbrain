...
export function searchHistoryKey(entry: {
  ...
  const ids = entry.genres
    ...
    .sort((a, b) => a - b)
    .join(',')
  return `${entry.titleKind}|${ids}|${entry.yearMin ?? ''}|${entry.yearMax ?? ''}|${entry.ratingMin}`
}

export function snapshotSearchHistory(filters: DiscoverFilters, genres: Genre[]): SearchHistoryInput {
...
export function applySearchHistory(
  ...
  return {
    ...
    page: 1,
    sortBy:
      entry.titleKind !== 'movie' && filters.sortBy === 'revenue.desc'
        ? 'popularity.desc'
        : filters.sortBy
  }
}

export function matchesSearchHistory(filters: DiscoverFilters, entry: SearchHistoryEntry): boolean {
  return (
    searchHistoryKey({
      titleKind: filters.titleKind,
...
export function historyYearsLabel(yearMin: number | null, yearMax: number | null): string {
  const now = new Date().getFullYear()
  const low = yearMin ?? 1900
  const high = yearMax ?? now
  if ((yearMin == null || yearMin <= 1900) && (yearMax == null || yearMax >= now)) return 'Any year'
...
export function normalizeSearchHistory(raw: unknown): SearchHistoryEntry[] {
  ...
  for (const item of raw) {
    ...
    if (value.titleKind !== 'movie' && value.titleKind !== 'tv' && value.titleKind !== 'miniseries') {
      continue
    }
    entries.push({
      id: value.id,
      ...
      ratingMin: typeof value.ratingMin === 'number' && Number.isFinite(value.ratingMin) ? value.ratingMin : 0
    })
  }
  return entries
}
...
function normalizeGenres(raw: unknown): SearchHistoryGenre[] {
  ...
  for (const item of raw) {
    ...
    genres.push({
      id: value.id,
      name: typeof value.name === 'string' && value.name.trim() ? value.name : `Genre ${value.id}`
    })
  }
  return genres
}
