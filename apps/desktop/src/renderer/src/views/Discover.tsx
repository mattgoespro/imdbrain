import { useEffect, useMemo, useRef, useState, type JSX, type ReactNode, type UIEvent } from 'react'
...
import { listSearchHistory, removeSearchHistory, saveSearchHistory } from '../lib/search-history-store'
...
export default function Discover({
  ...
  function onResultsScroll(event: UIEvent<HTMLDivElement>): void {
    ...
    if (el.scrollTop < Math.max(HISTORY_SCROLL_PX, el.clientHeight * HISTORY_SCROLL_VIEWPORT)) return
    recordSearchHistory()
  }

  function handleCardOpen(movie: MovieSummary): void {
    historySession.current.opens += 1
    if (historySession.current.opens >= HISTORY_OPEN_COUNT) recordSearchHistory()
    onOpen(movie)
  }

  function recordSearchHistory(): void {
    if (historySession.current.saved) return
    const snapshot = snapshotSearchHistory(filtersRef.current, genresRef.current)
  ...
  const activeHistoryId = history.find((entry) => matchesSearchHistory(filters, entry))?.id ?? null
  ...
  return (
    <section className="grid h-full min-h-0 flex-1 grid-cols-1 grid-rows-[auto_1fr_auto] inspect:grid-cols-[280px_minmax(0,1fr)_minmax(280px,400px)] inspect:grid-rows-none">
      <FilterPanel
        ...
        onApplyHistory={(entry) => setFilters(applySearchHistory(filters, entry))}
