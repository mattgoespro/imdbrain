...
export default function HistoryMenu({
  ...
  const [menuPos, setMenuPos] = useState({ top: 0, left: 0, width: MENU_WIDTH, maxHeight: 360 })
  ...
  function placeMenu(): void {
    ...
    const maxHeight = Math.min(420, Math.max(160, Math.max(spaceBelow, spaceAbove)))
  ...
  return (
    <div>
      <button
        ref={buttonRef}
        className={cn(btn('ghost', 'compact'), 'inline-flex items-center gap-1 pr-2')}
        type="button"
      ...
      {open
        ? createPortal(
            ...
            >
              {entries.length ? (
                <>
                  <p className="kicker sticky top-0 z-1 bg-raised px-2.5 py-2">Saved searches</p>
                  {entries.map((entry) => (
                    <HistoryOption
                      key={entry.id}
                      entry={entry}
                ...
                <p className="m-0 px-2.5 py-3 text-[12px] leading-[1.45] text-muted">
                  No saved searches yet. Scroll through results or open a couple of titles to record
                  the current filters.
                </p>
              )}
            </div>,
