...
export function registerIpc(
  ...
  hydrateImdbIds(store.listImdbIds());
  setImdbIdPersister((key, imdbId) => store.setImdbId(key, imdbId));

  ipcMain.handle("settings:get", () => store.getSettings());
  ipcMain.handle("settings:set", (_e, patch: Partial<Settings>) => {
    if (!isAppearanceOnlyPatch(patch)) genreCache = {};
    const next = store.setSettings(patch);
    applyWindowChrome(getWindow(), next);
    return next;
  });
  ipcMain.handle("tmdb:configured", () => client().configured());
  ipcMain.handle("tmdb:genres", (_e, mediaType?: MediaType) =>
    loadGenres(mediaType ?? "movie"),
  );
  ipcMain.handle("tmdb:providers", () => loadProviders());
  ipcMain.handle("tmdb:searchPeople", (_e, query: string) =>
    client().searchPeople(query),
  );
  ipcMain.handle("tmdb:searchKeywords", (_e, query: string) =>
    client().searchKeywords(query),
  );
  ipcMain.handle("tmdb:discover", (_e, filters: DiscoverFilters) =>
    discover(filters),
  );
  ipcMain.handle("tmdb:movie", (_e, id: number, mediaType?: MediaType) =>
    client().title(id, mediaType ?? "movie"),
  );
  ipcMain.handle("tmdb:movieMeta", (_e, movies: MovieSummary[]) =>
    enrichMovies(movies),
  );
  ipcMain.handle("library:list", () => store.listLibrary());
  ipcMain.handle("library:upsert", (_e, payload: LibraryUpsert) =>
    upsertLibrary(payload),
  );
  ipcMain.handle(
    "library:remove",
    (_e, tmdbId: number, mediaType?: MediaType) => {
...
async function discover(filters: DiscoverFilters): Promise<PagedMovies> {
  ...
  if (/^tt\d{5,}$/i.test(query)) {
    ...
    page.results = page.results.filter((movie) =>
      matchesLocalFilters(movie, filters, false),
    );
  } else {
    const extra = filters.sortBy === "match" && filters.page === 1 ? 3 : 0;
    const startPage = Math.max(1, filters.page);
    const pages = await Promise.all(
      Array.from({ length: 1 + extra }, (_, i) =>
        tmdb.discover(filters, startPage + i),
      ),
    );
  ...
  if (filters.hideWatched) {
    results = results.filter(
      (m) => library.get(titleKey(m))?.status !== "watched",
    );
  }
  if (filters.hideWatchlist) {
    results = results.filter(
      (m) => library.get(titleKey(m))?.status !== "watchlist",
    );
  }
  results = results.filter(
    (m) => library.get(titleKey(m))?.status !== "skipped",
  );

  seedLibraryMeta(tmdb, library);
  results = results.map((movie) =>
    tmdb.hydrateMovie(movie, library.get(titleKey(movie))),
  );
  results = await applyCachedImdbRatings(tmdb, results);
  results = results.filter(
    (movie) => !movie.imdbId || matchesRatingFilters(movie, filters),
  );
  ...
  if (ranked[0])
    void tmdb
      .title(ranked[0].tmdbId, ranked[0].mediaType)
      .catch(() => undefined);
...
function matchesLocalFilters(
  ...
  if (
    ...
    !filters.genres.some((id) => movie.genreIds.includes(id))
  )
    return false;
  if (filters.withoutGenres.some((id) => movie.genreIds.includes(id)))
    return false;
...
async function upsertLibrary(payload: LibraryUpsert): Promise<LibraryEntry[]> {
  ...
  const needsCredits =
    !(payload.movie as MovieDetails).directors &&
    !payload.movie.directorIds?.length;
  try {
  ...
  const entry: LibraryEntry = {
    ...
    certification:
      details?.certification ?? movie.certification ?? existing?.certification,
    ...
    directorIds:
      details?.directorIds ??
      movie.directorIds ??
      existing?.directorIds ??
      directors.map((d) => d.id),
    directorNames:
      details?.directorNames ??
      movie.directorNames ??
      existing?.directorNames ??
      directors.map((d) => d.name),
    castIds:
      details?.castIds ??
      movie.castIds ??
      existing?.castIds ??
      cast.slice(0, 8).map((c) => c.id),
    castNames:
      details?.castNames ??
      movie.castNames ??
      existing?.castNames ??
      cast.slice(0, 8).map((c) => c.name),
...
async function forYou(): Promise<ForYouResult> {
  ...
  const movieGenreIds = new Set(
    (await loadGenres("movie").catch(() => [] as Genre[])).map(
      (genre) => genre.id,
    ),
  );
  ...
  const tasks: Promise<MovieSummary[]>[] = seeds
    .slice(0, 6)
    .flatMap((seed) => [
      tmdb
        .recommendations(seed.tmdbId, 1, seed.mediaType ?? "movie")
        .catch(() => [] as MovieSummary[]),
      tmdb
        .similar(seed.tmdbId, 1, seed.mediaType ?? "movie")
        .catch(() => [] as MovieSummary[]),
    ]);
  ...
  const baseFilters: DiscoverFilters = {
    ...
    voteCountMin: defaults.voteCountMin,
    runtimeMin: null,
    runtimeMax: null,
    language: "",
    cast: [],
  ...
  if (topGenreIds[0]) {
    tasks.push(
      tmdb
        .discover(
          { ...baseFilters, genres: [topGenreIds[0]], yearMin: year - 8 },
          1,
        )
        .then((p) => p.results)
        .catch(() => [] as MovieSummary[]),
  ...
  for (const batch of batches) {
    for (const movie of batch) {
      ...
      if (!candidates.has(titleKey(movie)))
        candidates.set(titleKey(movie), movie);
    }
  }

  seedLibraryMeta(tmdb, library);

  const hydrated = [...candidates.values()].map((movie) =>
    tmdb.hydrateMovie(movie, library.get(titleKey(movie))),
  );
  const ranked = (await applyCachedImdbRatings(tmdb, hydrated))
...
async function importImdbCsv(): Promise<ImportProgress> {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    emitProgress(progress);
    try {
      const summary = await tmdb.findByImdb(
        row.imdbId,
        preferredMediaType(row.titleType),
      );
      if (!summary) {
        progress.skipped++;
        continue;
      ...
      if (entry && row.dateRated) {
        store.upsertEntry({
          ...entry,
          ratedAt: row.dateRated,
          watchedAt: row.dateRated,
        });
      }
      progress.imported++;
    } catch (error) {
...
async function applyCachedImdbRatings(
  tmdb: TmdbClient,
  movies: MovieSummary[],
): Promise<MovieSummary[]> {
  const resolved = movies.map((movie) => ({
    ...movie,
    imdbId:
      movie.imdbId ?? tmdb.imdbIdOf(movie.tmdbId, movie.mediaType ?? "movie"),
  }));
  const ids = resolved
    .map((movie) => movie.imdbId)
    .filter((id): id is string => Boolean(id));
  if (!ids.length) return resolved;
  return applyImdbRatings(tmdb, resolved);
}

async function applyImdbRatings(
  tmdb: TmdbClient,
  movies: MovieSummary[],
): Promise<MovieSummary[]> {
  const withIds = movies.every((movie) => movie.imdbId)
    ? movies
    : await tmdb.resolveImdbIds(movies);
  const ids = withIds
    .map((movie) => movie.imdbId)
    .filter((id): id is string => Boolean(id));
  const ratings = await new ImdbRatingsClient(
    ratingsApiUrl(store.getSettings().imdbApiUrl),
  ).lookup(ids);
  if (!ratings.size) return withIds;
  return withIds.map((movie) => {
    const row = movie.imdbId
      ? ratings.get(movie.imdbId.toLowerCase())
      : undefined;
    if (!row) return movie;
...
async function enrichMovies(
  ...
  const patched = await applyImdbRatings(tmdb, movies);
  const genres = await loadMergedGenres();
  const profile = buildProfile(store.listLibrary(), genres);
  const mode = store.getSettings().rankingMode;
  const patches: Record<string, MovieEnrichment> = {};
  for (const movie of patched) {
    ...
    patches[titleKey(movie)] = {
      ...
      imdbId: ranked.imdbId,
      voteAverage: ranked.voteAverage,
