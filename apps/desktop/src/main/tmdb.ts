...
export class TmdbClient {
  ...
  genres(mediaType: MediaType = 'movie'): Promise<{ genres: Genre[] }> {
    return this.get(mediaType === 'tv' ? '/genre/tv/list' : '/genre/movie/list')
  }
  ...
  async findByImdb(imdbId: string, preferred: MediaType = 'movie'): Promise<MovieSummary | null> {
  ...
  async searchTv(query: string, page: number, titleKind: TitleKind = 'tv'): Promise<TmdbPage> {
  ...
  async discover(filters: DiscoverFilters, page: number): Promise<TmdbPage> {
    if (mediaTypeOf(filters.titleKind) === 'tv') return this.discoverTv(filters, page)
  ...
  private async discoverTv(filters: DiscoverFilters, page: number): Promise<TmdbPage> {
    if (filters.genres.length > 1) {
      const pages = await Promise.all(
        filters.genres.map((id) => this.discoverTv({ ...filters, genres: [id] }, page))
      )
      return mergePages(pages, page, filters.sortBy)
    }
    ...
    return mapTvPage(data, filters.titleKind === 'miniseries' ? 'miniseries' : 'tv')
  }

  async title(id: number, mediaType: MediaType = 'movie'): Promise<MovieDetails> {
  ...
  async tv(id: number): Promise<MovieDetails> {
    return this.loadDetails('tv', id, () =>
      this.get<TmdbTvDetails>(`/tv/${id}`, {
        append_to_response: 'aggregate_credits,credits,external_ids,keywords,content_ratings'
  ...
  private async loadDetails(
    ...
    const request = fetchDetails()
      .then((details) => {
        ...
        rememberMeta(key, details.runtime ?? null, details.certification ?? null)
        rememberCounts(key, details.seasonCount ?? null, details.episodeCount ?? null)
  ...
  async recommendations(id: number, page = 1, mediaType: MediaType = 'movie'): Promise<MovieSummary[]> {
    const path = mediaType === 'tv' ? `/tv/${id}/recommendations` : `/movie/${id}/recommendations`
    if (mediaType === 'tv') {
  ...
  async similar(id: number, page = 1, mediaType: MediaType = 'movie'): Promise<MovieSummary[]> {
  ...
  rememberRuntime(id: number, runtime?: number, mediaType: MediaType = 'movie'): void {
    rememberMeta(cacheKey(mediaType, id), runtime, undefined)
  }

  rememberCertification(id: number, certification?: string, mediaType: MediaType = 'movie'): void {
    rememberMeta(cacheKey(mediaType, id), undefined, certification)
  }

  rememberCredits(id: number, credits?: CreditsMeta | null, mediaType: MediaType = 'movie'): void {
    storeCredits(cacheKey(mediaType, id), credits)
  }

  rememberImdbId(id: number, imdbId?: string, mediaType: MediaType = 'movie'): void {
    storeImdbId(cacheKey(mediaType, id), imdbId)
  }

  async externalIds(id: number, mediaType: MediaType = 'movie'): Promise<string | undefined> {
    const key = cacheKey(mediaType, id)
    ...
    const request = this.get<{ imdb_id?: string | null }>(
      mediaType === 'tv' ? `/tv/${id}/external_ids` : `/movie/${id}/external_ids`
    )
      .then((data) => {
        const imdbId = normalizeImdbId(data.imdb_id)
        imdbIdCache.set(key, imdbId ?? null)
  ...
  certificationOf(id: number, mediaType: MediaType = 'movie'): string | undefined {
    return certificationCache.get(cacheKey(mediaType, id)) ?? undefined
  }

  creditsOf(id: number, mediaType: MediaType = 'movie'): CreditsMeta | undefined {
    const key = cacheKey(mediaType, id)
    return creditsCache.get(key) ?? creditsFromDetails(detailsCache.get(key)) ?? undefined
  }

  hydrateMovie(
    movie: MovieSummary,
    library?: { runtime?: number; certification?: string; seasonCount?: number; episodeCount?: number } & Partial<CreditsMeta>
  ): MovieSummary {
    ...
    return {
      ...
      runtime: movie.runtime ?? this.runtimeOf(movie.tmdbId, mediaType) ?? library?.runtime,
      certification: movie.certification ?? this.certificationOf(movie.tmdbId, mediaType) ?? library?.certification,
      directorIds: firstCredits(movie.directorIds, credits?.directorIds, library?.directorIds),
      directorNames: firstCredits(movie.directorNames, credits?.directorNames, library?.directorNames),
      castIds: firstCredits(movie.castIds, credits?.castIds, library?.castIds),
      castNames: firstCredits(movie.castNames, credits?.castNames, library?.castNames),
      seasonCount: movie.seasonCount ?? seasonCache.get(key) ?? library?.seasonCount ?? undefined,
      episodeCount: movie.episodeCount ?? episodeCache.get(key) ?? library?.episodeCount ?? undefined
    }
  }
  ...
  async prefetchRuntimes(ids: number[]): Promise<void> {
    await this.prefetchMovieMeta(ids.map((tmdbId) => ({ tmdbId, mediaType: 'movie' as const, titleKind: 'movie' as const })))
  }

  async prefetchMovieMeta(movies: Array<Pick<MovieSummary, 'tmdbId' | 'mediaType'>>): Promise<Record<string, MovieMeta>> {
    const unique = [...new Map(movies.map((movie) => [cacheKey(movie.mediaType ?? 'movie', movie.tmdbId), movie])).values()]
    const missing = unique.filter((movie) => {
      const key = cacheKey(movie.mediaType ?? 'movie', movie.tmdbId)
      return !runtimeCache.has(key) || !certificationCache.has(key) || !creditsCache.has(key)
    })
    const queue = [...missing]
    const workerCount = Math.min(6, queue.length)
    await Promise.all(
      Array.from({ length: workerCount }, async () => {
        while (queue.length) {
          const movie = queue.shift()
          if (movie == null) return
          await this.ensureMeta(movie.tmdbId, movie.mediaType ?? 'movie')
        }
      })
    )
    return Object.fromEntries(
      unique.map((movie) => {
        const mediaType = movie.mediaType ?? 'movie'
        return [cacheKey(mediaType, movie.tmdbId), this.metaOf(movie.tmdbId, mediaType)]
      })
    )
  }
  ...
  private async ensureMeta(id: number, mediaType: MediaType): Promise<void> {
    const key = cacheKey(mediaType, id)
    if (runtimeCache.has(key) && certificationCache.has(key) && creditsCache.has(key)) return
    ...
    const request = (
      mediaType === 'tv'
        ? this.get<{
          ...
          }>(`/tv/${id}`, { append_to_response: 'credits,aggregate_credits,content_ratings' })
        : this.get<{ runtime: number | null; release_dates?: TmdbReleaseDates; credits?: TmdbDetails['credits'] }>(
            `/movie/${id}`,
      ...
      .then((data) => {
        if (mediaType === 'tv') {
          ...
          rememberMeta(key, episodeRuntime(tv.episode_run_time), pickTvCertification(tv.content_ratings, this.region) ?? null)
          rememberCounts(key, tv.number_of_seasons ?? null, tv.number_of_episodes ?? null)
          storeCredits(key, creditsFromTvPayload(tv))
          return
        }
        const movie = data as { runtime: number | null; release_dates?: TmdbReleaseDates; credits?: TmdbDetails['credits'] }
        rememberMeta(
          key,
...
function toTvDetails(data: TmdbTvDetails, region: string): MovieDetails {
  ...
  const crewDirectors = (data.aggregate_credits?.crew ?? data.credits?.crew ?? [])
    .filter((person) =>
      'jobs' in person
  ...
  const directors = (creators.length ? creators : crewDirectors).map((person) => ({
    id: person.id,
    name: person.name
  }))
  const aggregateCast = data.aggregate_credits?.cast ?? []
  const cast: CastMember[] = (aggregateCast.length ? aggregateCast : data.credits?.cast ?? [])
    .slice(0, 16)
    .map((person, index) => ({
      id: person.id,
      name: person.name,
  ...
  const genres = data.genres ?? []
  const titleKind: TitleKind = /mini/i.test(data.type ?? '') ? 'miniseries' : 'tv'
  return {
...
function creditsFromDetails(details?: MovieDetails): CreditsMeta | undefined {
  ...
  if (!details.directorIds?.length && !details.castIds?.length) return undefined
  return {
...
function creditsFromPayload(credits?: TmdbDetails['credits']): CreditsMeta | null {
...
function creditsFromTvPayload(data: {
  ...
  const cast = (data.aggregate_credits?.cast ?? data.credits?.cast ?? []).slice(0, 8)
  if (!directors.length && !cast.length) return null
  return {
    directorIds: directors.map((person) => person.id),
    directorNames: directors.map((person) => person.name),
...
function firstCredits<T>(...candidates: Array<T[] | undefined>): T[] | undefined {
  return candidates.find((value) => value != null && value.length > 0)
}

function rememberMeta(key: string, runtime?: number | null, certification?: string | null): void {
  if (runtime != null && runtime > 0 && !runtimeCache.has(key)) runtimeCache.set(key, runtime)
  if (runtime === null && !runtimeCache.has(key)) runtimeCache.set(key, null)
  if (certification && !certificationCache.has(key)) certificationCache.set(key, certification)
  if (certification === null && !certificationCache.has(key)) certificationCache.set(key, null)
}

function rememberCounts(key: string, seasons?: number | null, episodes?: number | null): void {
  if (seasons != null && seasons > 0 && !seasonCache.has(key)) seasonCache.set(key, seasons)
  if (seasons === null && !seasonCache.has(key)) seasonCache.set(key, null)
  if (episodes != null && episodes > 0 && !episodeCache.has(key)) episodeCache.set(key, episodes)
  if (episodes === null && !episodeCache.has(key)) episodeCache.set(key, null)
}

function pickCertification(data: TmdbReleaseDates | undefined, region: string): string | undefined {
  const groups = data?.results ?? []
  if (!groups.length) return undefined
  const wanted = (region || 'US').toUpperCase()
  const ordered = [
    ...groups.filter((group) => group.iso_3166_1 === wanted),
    ...(wanted !== 'US' ? groups.filter((group) => group.iso_3166_1 === 'US') : []),
    ...groups.filter((group) => group.iso_3166_1 !== wanted && group.iso_3166_1 !== 'US')
  ]
  for (const group of ordered) {
    const dates = group.release_dates ?? []
    const theatrical = dates.find((entry) => (entry.type === 2 || entry.type === 3) && cleanCert(entry.certification))
    const any = dates.find((entry) => cleanCert(entry.certification))
    const value = cleanCert(theatrical?.certification) ?? cleanCert(any?.certification)
    if (value) return value
  }
  return undefined
}

function pickTvCertification(data: TmdbContentRatings | undefined, region: string): string | undefined {
  const groups = data?.results ?? []
  if (!groups.length) return undefined
  const wanted = (region || 'US').toUpperCase()
  const ordered = [
...
function storeImdbId(key: string, imdbId?: string | null): void {
  const normalized = normalizeImdbId(imdbId)
  if (normalized && !imdbIdCache.has(key)) imdbIdCache.set(key, normalized)
}
